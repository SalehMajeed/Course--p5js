...
class Bouncing_ball {
	...
	get_color() {
		let color = {
			r: Math.floor(random(6, 255)),
			g: Math.floor(random(6, 255)),
			b: Math.floor(random(6, 255)),
		};
		...
		for (let i = -5; i <= 5; i++) {
			let color_code = `r${color.r + i}g${color.g + i}b${color.b + i}`;

			color_values.push(color_code);
		}
	...
	move() {
		...
		if (this.x <= _canvas.box.width - this.size && this.y <= _canvas.box.height - this.size) {
			...
			this.x <= _canvas.box.width - this.size &&
			this.y >= this.canvas_height - _canvas.box.height + this.size
		) {
			// bottom left
			this.canvas_width = _canvas.box.width;
			this.canvas_height = _canvas.box.height;
			...
			this.x >= this.canvas_width - _canvas.box.width + this.size &&
			this.y <= _canvas.box.height - this.size
		) {
			// top right
			this.canvas_width = _canvas.box.width;
			this.canvas_height = _canvas.box.height;
